...
class BaseModel():
    ...
    def get_errors(self):
        ...
        errors = OrderedDict([
            ('err_d_cls', self.err_d_cls.item()),
            ('err_d_reg', self.err_d_reg.item()),
    ...
    def train_one_epoch(self):
        ...
        self.netg.train() # sets the module in training mode
        epoch_iter = 0
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                path_D = "./output/{}/{}/train/weights/netD.pth".format(self.name.lower(), self.opt.dataset)
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                time_i = time.time()
                self.set_input(data)
                self.fake, latent_i= self.netg(self.input)
                fake_cls, _, fake_reg = self.netd(self.fake.detach())
                latent_o = fake_reg
...
class Ganomaly(BaseModel):
    ...
    def forward_d(self):
        ...
        """
        self.pred_real, self.feat_real, self.reg_real = self.netd(self.input)
        self.pred_fake, self.feat_fake, self.reg_fake = self.netd(self.fake.detach())
    ...
    def backward_d(self):
        ...
        # Regressor Loss
        self.err_real_reg = self.l_enc(self.reg_real, self.netg(self.input)[1])
        self.err_fake_reg = self.l_enc(self.reg_fake, self.netg(self.input)[1])
        self.err_d_reg = (self.err_real_reg + self.err_fake_reg) * 0.5

        # NetD Loss & Backward-Pass
        self.err_d_cls = (self.err_d_real + self.err_d_fake) * 0.5
        #return self.err_d_cls, self.err_d_reg
    ...
    def optimize_params(self):
        ...
        self.err_d_cls.backward(retain_graph=True)
        self.optimizer_d.step()

        self.optimizer_d.zero_grad()
        self.err_d_reg.backward()
        self.optimizer_d.step()
